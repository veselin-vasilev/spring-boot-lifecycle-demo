...
public class BeanListenerConfig {
  ...
  public void appStarted(ApplicationStartedEvent event) {
    LOGGER.info(
        "The ApplicationStartedEvent has been fired. Time taken: {}. The context was refreshed, but no command-line or application runners have been called yet.",
        event.getTimeTaken());
  }

  @EventListener
  public void availabilityChange(AvailabilityChangeEvent<? extends AvailabilityState> event) {
    LOGGER.info(
        "An AvailabilityChangeEvent has been fired. The state is: {} of type: {}",
        event.getState(),
  ...
  public void appReady(ApplicationReadyEvent event) {
    LOGGER.info(
        "The ApplicationReadyEvent has been fired. Time taken: {}. Command-line and application runners have been called and are completed.",
        event.getTimeTaken());
  }
  ...
  public void webServerInitialized(WebServerInitializedEvent event) {
    LOGGER.info(
        "The WebServer is ready! It will listen on port: {}", event.getWebServer().getPort());
  }

  @EventListener
  public void contextClosed(ContextClosedEvent event) {
    LOGGER.info(
        "The context with id = {} has been closed. Time to do shutdown operations.",
